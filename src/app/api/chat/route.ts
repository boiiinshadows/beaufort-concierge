    ...
    tools: {
      ...
      captureLeadContact: {
        ...
        inputSchema: z.object({
          ...
          // Enrichment — include all intent signals collected during conversation
          purpose: z.enum(['investment', 'own use', 'undecided']).optional(),
          buyer_type: z.enum(['investor', 'owner-occupant', 'undecided']).optional(),
          budget_min: z.number().optional().describe('Minimum budget in USD'),
          budget_max: z.number().optional().describe('Maximum budget in USD'),
          preferred_location: z.string().optional().describe('Preferred area e.g. East Legon, Airport Residential'),
          viewing_property: z.string().optional().describe('Property they expressed interest in'),
        ...
        execute: async ({ first_name, last_name, phone, email, purpose, buyer_type, budget_min, budget_max, preferred_location, viewing_property, conversation_summary, lead_score }) => {
            ...
            if (existing) {
              ...
              const { error } = await supabase
                ...
                .update({
                  ...
                  buyer_type: buyer_type || existing.buyer_type,
                  budget_min: budget_min ?? existing.budget_min,
                  budget_max: budget_max ?? existing.budget_max,
                  preferred_location: preferred_location || existing.preferred_location || existing.location_preference,
                  location_preference: preferred_location || existing.preferred_location || existing.location_preference,
                  viewing_property: viewing_property || existing.viewing_property,
                  interested_property: viewing_property || existing.interested_property,
              ...
              const { error } = await supabase.from('leads').insert({
                ...
                viewing_property,
                interested_property: viewing_property,
